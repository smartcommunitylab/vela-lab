...
static const spi_device_t spi_dev = {
  ...
  .spi_bit_rate = 1000000, // TODO: HAS TO BE A COMMON PARAMETER
  .spi_pha = 0,
  .spi_pol = 0
};
...
void uart_util_ack_error(ack_wait_t* ack_wait_data) {
	...
	if(no_of_attempts < MAX_ATTEMPTS){
		...
		reset_nordic(); //when the nordic will reboot it will generate the uart_ready evt. When the TI receives it, it restart the FSM
	}
}
