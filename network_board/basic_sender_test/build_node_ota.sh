...
OTA_UUID=0xabcd1234
OTA_PRE_VERIFIED=1  #NB: for GOLDEN IMAGE and in general when firmware is going to be loaded through the debugger, set this to 1. For OTA updates set this to 0 - ATTENTION: this not would be true in an ideal world. It appeared that overwriting the OTA metadata onboard requires too much ram that we don't have. For this reason set always set OTA_PRE_VERIFIED=1, the ota will be anyway verified, and if the CRC doesn't match the ota slot will be erased (see verify_ota_slot(...) in ota.c).

CLEAR_OTA_SLOTS=0
BURN_GOLDEN_IMAGE=0
recompile_bootloader=0 #use 1 to recompile the bootloader, otherwise the already compiled one will be used. If you don't set this to 1 (and carefully check the value of CLEAR_OTA_SLOTS and BURN_GOLDEN_IMAGE, in a deployment both should be 0)
# ************************************************ BUILDING ************************************************
