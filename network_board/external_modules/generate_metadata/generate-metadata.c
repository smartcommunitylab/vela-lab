...
main(int argc, char *argv[]) {
  ...
  printf("metadata.crc: 0x%04x\n", metadata.crc );
  if (firmware_verified) {
    printf("metadata.crc_shadow: 0x%04x - (the image IS preverified, adeguate for being flashed with the debugger)\n", metadata.crc_shadow );
  }else{
    printf("metadata.crc_shadow: 0x%04x - (the image IS NOT preverified, adeguate for OTA download)\n", metadata.crc_shadow );
  }
  printf("metadata.size: 0x%08x\n",metadata.size);
  printf("metadata.uuid: 0x%08x\n",metadata.uuid);
  printf("metadata.version: 0x%04x\n", metadata.version );
