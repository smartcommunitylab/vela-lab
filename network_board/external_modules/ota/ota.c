...
get_ota_slot_metadata( uint8_t ota_slot, OTAMetadata_t *ota_slot_metadata )
{
  ...
  eeprom_access = ext_flash_read(NULL,ota_image_address, OTA_METADATA_LENGTH, (uint8_t *)ota_slot_metadata);
...
verify_current_firmware( OTAMetadata_t *current_firmware_metadata )
{
  ...
  txt_len=sprintf(txt_buff,"\nfirmware_end_address: 0x");
  boot_board_write_uart(txt_buff,txt_len);
  boot_board_print_uin32_t_hex(firmware_end_address);
...
/*
#if defined(ENABLE_BOOTLOADER_VERBOSITY) && ENABLE_BOOTLOADER_VERBOSITY==1
    if(firmware_address!=0x2000 + OTA_METADATA_SPACE){
      txt_len=sprintf(txt_buff,"\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b");
      boot_board_write_uart(txt_buff,txt_len);
    }

    txt_len=sprintf(txt_buff,"address: 0x");
    boot_board_write_uart(txt_buff,txt_len);
    boot_board_print_uin32_t_hex(firmware_address);

    txt_len=sprintf(txt_buff,"  |  content: 0x");
    boot_board_write_uart(txt_buff,txt_len);
    uint32_t __word = ((uint32_t)_word[3]<<24) + ((uint32_t)_word[2]<<16) + ((uint32_t)_word[1]<<8) + ((uint32_t)_word[0]);
    boot_board_print_uin32_t_hex(__word);
#endif
*/
  ...
  //  (4) Finally, update Metadata stored in internal flash
...
erase_extflash_page( uint32_t ext_address )
{
  ...
  eeprom_access = ext_flash_erase( NULL,ext_address, FLASH_PAGE_SIZE );
  if(!eeprom_access) {
    PRINTF("[external-flash]:\tError - Could not erase EEPROM.\n");
    ext_flash_close(NULL);
    return -1;
...
update_firmware_page( uint32_t source_page, uint32_t destination_page )
{
  ...
  eeprom_access = ext_flash_read( NULL,source_page, FLASH_PAGE_SIZE, (uint8_t *)&page_data);
...
store_firmware_data( uint32_t ext_address, uint8_t *data, size_t data_length )
{
  ...
  int eeprom_access;
  eeprom_access = ext_flash_open(NULL);
  ...
  eeprom_access = ext_flash_write(NULL,ext_address, data_length, data );
