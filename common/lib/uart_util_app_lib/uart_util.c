...
void serial_evt_handle(app_uart_evt_t * p_event);
...
void process_uart_rx_data(uint8_t *uart_frame){
	...
	case waiting_start_seq:
		...
		expected_lenght = 0;
#ifndef CONTIKI
		while(app_uart_get(&uart_frame[idx]) != NRF_SUCCESS);
    written_size += 1; bsp_board_led_on(0);
    ret = NRF_SUCCESS;
...
void serial_evt_handle(app_uart_evt_t * p_event)
{
	...
	case (APP_UART_DATA_READY):
		...
		break;
	case (APP_UART_FIFO_ERROR): //should be triggered only if the FIFO is not enabled
		break;
	default:
		break;
	}
...
#ifndef CONTIKI
  for (int i = 0; i < idx; i++) {
	  while(app_uart_put(uart_frame[i]) != NRF_SUCCESS);     
  }
  bsp_board_led_on(1);
  ret = NRF_SUCCESS;
#else
...
void uart_util_initialize(void){
    ...
    uint32_t ret = app_timer_create(&m_ack_timer_id,
